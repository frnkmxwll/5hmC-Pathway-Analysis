...
# install  consensusSeekeR package
...
# load metadata file
metadata_file <- "~/5hmC-Pathway-Analysis/Raw Input/Working Inputs/all_comparisons_9_03_2024.csv" #7_2_2024.csv" for v1 analysos or 9_03_2024 for v2 analysis
metadata <- read.csv(metadata_file, stringsAsFactors = TRUE)

# Keep only rows where condition == "AMneg" or "AMpos"
metadata <- metadata[metadata$condition %in% c("PM_positive", "PM_negative","HEALTHY"),]
# Set first column to row names, and drop first column
...
# drop excluded samples from metadata
metadata <- metadata[!rownames(metadata) %in% excluded_samples,]
included_samples <- rownames(metadata)[!rownames(metadata) %in% excluded_samples]
# Select random 10 files
#included_samples <- sample(included_samples, 10)

# create column called "ID" equal to row names, then drop rownames
...
merged_peak_file_saf <- file.path(output_dir, "merged_peaks_10perc_pe5_10162024.saf") # 072224_gene.saf for v1 analysis # 090324_gene.saf for v2 analysis
merged_peak_file_saf_anno <- file.path(output_dir, "merged_peaks_10perc_pe5_10162024_gene.saf") # 072224_gene.saf for v1 analysis # 090324_gene.saf for v2 analysis
...
final_peaks <- final_peaks[grepl("chr[1-9]|chr1[0-9]|chr2[0-2]|chrX", final_peaks$seqnames),]
# drop rows where seqnames contain "random"
final_peaks <- final_peaks[!grepl("random", final_peaks$seqnames),]
###
###

# print row 96799 of final_peaks
#print(final_peaks[96799,])
...
if(FALSE){
  ...
  if (!file.exists(merged_peak_file_saf)) {
    ...
    # make sure peak_df start and end are numbers
...
raw_counts_file_1 <- "~/peritoneal_processing/macs_6_30_2024_p_1e5/CRC_ADE_HEALTHY/ReadsCount_10percent_macs3_pe5_092824_peaks.txt" # ReadsCount_10percent_macs3_pe5_092824_zhang_genes.txt" # 092824 for v3 analysis #090324 for v2 analysis # 072224 for v1 analysis

counts1_sample_pathname <- "../../peritoneal_processing/trimmed_data_bam_9_18_2023/"
counts1_sample_extension <- "_bowtie2.bam"

file_version <- "10per_10172024_hmr_combat_pm_healthy"

### Load Sample File
sample_file <- "~/5hmC-Pathway-Analysis/Raw Input/Working Inputs/all_comparisons_9_03_2024.csv"
#excluded_samples <- c("KT026", "KT26","KT027","KT27")
selected_conds <- list("PM_positive", "PM_negative", "HEALTHY")
...
freq_plot <- ggplot(plot_data, aes(x = factor(ReadThreshold), y = Percentage)) +
  ...
  geom_vline(xintercept = 4.5, linetype = "dashed", color = "#1F78B4", size = 1) +  # Vertical dashed line at x=20 (position 4 when step size is 5)
  annotate("text", x = 4.8, y = max(plot_data$Percentage) + 2, 
           label = "90% samples with >= 20 reads", color = "#1F78B4", angle = 90, vjust = 0, hjust = 0.9) +
  scale_fill_identity()  # Use the custom color scale

### END ON FREQUENCY OF COUNTS
...
if(grepl("zhang_genes", raw_counts_file_1)){
  ...
  keep <- rowSums(counts_final_sample[, sample_names] >= 0) >= 0 * ncol(counts_final_sample[, sample_names])
  ...
  # Drop rows from counts_data that don't have at least 90% of columns with 10 reads or more
  keep <- rowSums(counts_final_sample[, sample_names] >= 20) >= 0.9 * ncol(counts_final_sample[, sample_names])
  # Print kept versus total
  print(paste("Kept", sum(keep), "of", nrow(counts_final_sample), "rows"))
  counts_final_sample <- counts_final_sample[keep,]
}
...
ggsave(paste("./Output/Raw Data Processing/", class1_name, "_", class2_name, "_", file_version, "/", "frequency_plot.png", sep = ""), freq_plot, width = 8, height = 6, units = "in")
ggsave(paste("./Output/Raw Data Processing/", class1_name, "_", class2_name, "_", file_version, "/", "length_plot.png", sep = ""), length_plot, width = 8, height = 6, units = "in")
# also save as svg
ggsave(paste("./Output/Raw Data Processing/", class1_name, "_", class2_name, "_", file_version, "/", "frequency_plot.svg", sep = ""), freq_plot, width = 8, height = 6, units = "in")
ggsave(paste("./Output/Raw Data Processing/", class1_name, "_", class2_name, "_", file_version, "/", "length_plot.svg", sep = ""), length_plot, width = 8, height = 6, units = "in")


### --- GENERATE PEAKS PER GENOMIC FEATURE TYPE ---
source("~/5hmC-Pathway-Analysis/R Code/repeated_glmnet_funcs_v1.R")
## Generate list of all peak files in comparison
### Load Sample File
sample_file <- "~/5hmC-Pathway-Analysis/Raw Input/Working Inputs/all_comparisons_9_03_2024.csv"
#excluded_samples <- c("KT026", "KT26","KT027","KT27")
selected_conds <- list("PM_positive","PM_negative","HEALTHY")
...
peak_files <- list.files("~/peritoneal_processing/macs_6_30_2024_p_1e5/CRC_ADE_HEALTHY", pattern = ".xls", full.names = TRUE)
...
process_peak_file <- function(peak_file, merged_peak_file_gr, intersect_filter = TRUE) {
  output_fn = paste0(dirname(peak_file), "/annotated_peaks_filt_10_16_24/", gsub(".xls", ".csv", basename(peak_file)))
  # replace "/CRC_ADE_HEALTHY" with nothing
  output_fn = gsub("/CRC_ADE_HEALTHY", "", output_fn)
...
annotated_peak_files <- list.files("~/peritoneal_processing/macs_6_30_2024_p_1e5/annotated_peaks_filt_10_16_24", pattern = "peaks.csv", full.names = TRUE)
...
if (file.exists("~/peritoneal_processing/macs_6_30_2024_p_1e5/annotated_peaks_filt_10_16_24/total_reads_10_16_2024.csv")) {
  total_reads <- read.csv("~/peritoneal_processing/macs_6_30_2024_p_1e5/annotated_peaks_filt_10_16_24/total_reads_10_16_2024.csv", row.names = 1)
} else {
  ...
  write.csv(total_reads, file = "~/peritoneal_processing/macs_6_30_2024_p_1e5/annotated_peaks_filt_10_16_24/total_reads_10_16_2024.csv")
}

# Extract the 'x' column from total_reads as a named vector
total_reads_vector <- total_reads$x
names(total_reads_vector) <- rownames(total_reads)
...
peak_counts_melted$value <- as.integer(peak_counts_melted$value)
...
custom_colors <- c("PM_negative" = "#3e75b0", "PM_positive" = "#d2321f","HEALTHY" = "grey")  # Cornflower Blue and Tomato
...
# Perform t-tests or Wilcoxon tests based on assumptions
# For this example, let's proceed with t-tests
...
for (var in unique(peak_counts_melted$variable)) {
  ...
  test_results$condition1 <- rownames(test_results)
  
  # Reshape to long format
  test_results_long <- test_results %>%
    pivot_longer(cols = -condition1, names_to = "condition2", values_to = "p_value") %>%
    mutate(variable = var) %>%
...
test_results_combined <- test_results_df %>%
  left_join(summary_stats, by = c("variable", "condition1" = "condition")) %>%
  rename(samples1 = samples, mean1 = mean, sd1 = sd, median1 = median, IQR1 = IQR) %>%
  left_join(summary_stats, by = c("variable", "condition2" = "condition")) %>%
  rename(samples2 = samples, mean2 = mean, sd2 = sd, median2 = median, IQR2 = IQR)

# Reorder columns
test_results_combined <- test_results_combined %>%
  select(variable, condition1, condition2,
         samples1, mean1, sd1, median1, IQR1,
         samples2, mean2, sd2, median2, IQR2,
         p_value) %>%
  arrange(variable, condition1, condition2)

# View the final results
#print(test_results_combined,n=50)
## end sig test

write.csv(test_results_combined, file = "~/5hmC-Pathway-Analysis/Output/feature_type/peak_counts_per_genomic_feature_type_PMposPMneg_HEALTHY_10_14_24.csv", row.names = FALSE)
# save png and svg files to ~/5hmC-Pathway-Analysis/Output/feature_count/ 
ggsave("~/5hmC-Pathway-Analysis/Output/feature_type/peak_counts_per_genomic_feature_type_PMposPMneg_HEALTHY_10_14_24.png", plot = p, width = 10, height = 3, units = "in", dpi = 300)
ggsave("~/5hmC-Pathway-Analysis/Output/feature_type/peak_counts_per_genomic_feature_type_PMposPMneg_HEALTHY_10_14_24.svg", plot = p, width = 10, height = 3, units = "in")
...
p2 <- ggplot(peak_counts_melted, aes(x = condition, y = value, fill = condition)) +
  ...
  geom_signif(
    ...
    test = "t.test",  # You can change this to "wilcox.test" if you prefer
    step_increase = 0.1,
    tip_length = 0.01,
  ) +
  theme_minimal() +
...
# Save the plot
ggsave("~/5hmC-Pathway-Analysis/Output/feature_type/peak_counts_per_genomic_feature_type_subplots_with_ggsignif_adjusted_8_11_24.png", 
       plot = p2, width = 15, height = 12, units = "in", dpi = 300)
ggsave("~/5hmC-Pathway-Analysis/Output/feature_type/peak_counts_per_genomic_feature_type_subplots_with_ggsignif_adjusted_8_11_24.svg", 
...
ggsave("~/5hmC-Pathway-Analysis/Output/feature_type/Enhancer_Type_Distribution_10_14_24.png", enh_plot, width = 4, height = 6, units = "in", dpi = 300)
ggsave("~/5hmC-Pathway-Analysis/Output/feature_type/Enhancer_Type_Distribution_10_14_24.svg", enh_plot, width = 4, height = 6, units = "in")
ggsave("~/5hmC-Pathway-Analysis/Output/feature_type/Location_Type_Distribution_10_14_24.png", pie_plot, width = 6, height = 4, units = "in", dpi = 300)
ggsave("~/5hmC-Pathway-Analysis/Output/feature_type/Location_Type_Distribution_10_14_24.svg", pie_plot, width = 6, height = 4, units = "in")
...
ROOT_FOLDER <- "~/5hmC-Pathway-Analysis/Output/Raw Data Processing/HEALTHY_PM_positive_10per_080424_hmr_nocombat_all/"
FILENAME_TRUNK <- "HEALTHY_PM_positive_DESeq2"
OUTPUT_FOLDER_METAGENE <- "~/5hmC-Pathway-Analysis/Output/metagene"
BIGWIG_FOLDER <- "~/peritoneal_processing/trimmed_data_bw_9_18_2023_bigwig/"
...
metagene_analysis <- function(
  ...
  create_combined_metagene_plot <- function(plot_data) {
...
ggsave(paste0("~/5hmC-Pathway-Analysis/Output/metagene/", "HEALTHY_PM_positive_DESeq2_metagene_plot.png"), plot, width = 6, height = 4, units = "in", dpi = 300)
# save plot to svg
ggsave(paste0("~/5hmC-Pathway-Analysis/Output/metagene/", "HEALTHY_PM_positive_DESeq2_metagene_plot.svg"), plot, width = 6, height = 4, units = "in")
...
if(TRUE){
  ...
  condition_colors <- c("PM_positive" = "#E31A1C", "PM_negative" = "#1F78B4", "HEALTHY" = "#CCCCCC")
